...
export class ReportesPagosBancariosPage {
  ...
  private filtros: { periodo: string; tenantId: number; carteraId: number; subcarteraId: number } | null = null;

  constructor(private readonly reportePagosBancariosService: ReportePagosBancariosService) {}

  consultar(filtros: { periodo: string; tenantId: number; carteraId: number; subcarteraId: number }): void {
  ...
  private cargarPagina(pagina: number): void {
    ...
    this.reportePagosBancariosService.obtenerReporte(
      this.filtros.periodo,
      this.filtros.tenantId,
      this.filtros.carteraId,
      this.filtros.subcarteraId,
      pagina
    ).subscribe({
