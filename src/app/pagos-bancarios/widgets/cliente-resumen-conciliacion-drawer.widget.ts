...
@Component({
  ...
  template: `
    @if (open) {
      <div class="fixed inset-0 z-50 flex justify-end bg-black/40" (click)="close.emit()">
        <aside class="h-full w-full max-w-2xl overflow-y-auto bg-white shadow-2xl dark:bg-slate-900" (click)="$event.stopPropagation()">
          <div class="sticky top-0 z-10 border-b border-slate-200 bg-white px-5 py-4 dark:border-slate-700 dark:bg-slate-900">
            <div class="flex items-start justify-between gap-4">
              <div>
                <p class="text-xs font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">Resumen de conciliación</p>
                <h2 class="mt-1 text-xl font-bold text-slate-900 dark:text-white">{{ resumen?.nombreCliente || 'Cliente sin nombre' }}</h2>
          ...
          <div class="p-5">
            @if (loading) {
              ...
              <div class="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                ...
                <div class="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
                  <p class="text-xs text-slate-500 dark:text-slate-400">Contexto</p>
                  <p class="mt-1 text-xs font-semibold text-slate-700 dark:text-slate-300">{{ resumen.tenantId }}/{{ resumen.carteraId }}/{{ resumen.subcarteraId }}</p>
                </div>
              </div>

              <div class="space-y-3">
                @for (promesa of resumen.promesas; track promesa.idGestion) {
                  <details class="overflow-hidden rounded-xl border border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800/50" open>
                    <summary class="cursor-pointer px-4 py-3">
                      <div class="inline-flex w-full items-center justify-between gap-3 align-middle">
                        <div>
                          <p class="font-semibold text-slate-900 dark:text-white">Gestión {{ promesa.idGestion }} · {{ promesa.estadoPago }}</p>
                          <p class="text-xs text-slate-500 dark:text-slate-400">{{ promesa.fechaGestion }} · {{ promesa.nombreAgente || 'Sin agente' }}</p>
                        </div>
                        <div class="text-right text-sm font-semibold text-slate-700 dark:text-slate-200">S/ {{ formatMoney(promesa.montoPagadoReal) }} / S/ {{ formatMoney(promesa.montoPromesa) }}</div>
                      </div>
                    </summary>
                      ...
                      @for (cuota of promesa.cuotas; track cuota.cuotaId) {
                        <div class="rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-900">
                          <div class="flex flex-wrap items-center justify-between gap-2">
                            <p class="font-semibold text-slate-800 dark:text-slate-100">Cuota {{ cuota.numeroCuota }} · {{ cuota.estado }}</p>
                            <p class="text-sm text-slate-600 dark:text-slate-300">{{ cuota.fechaPromesa }} · S/ {{ formatMoney(cuota.montoPromesa) }}</p>
                          </div>
                            ...
                            <table class="min-w-full text-xs">
                              <thead class="text-left text-slate-500 dark:text-slate-400">
                                <tr>
                                  ...
                                  <th class="py-1 pr-3">Operación</th>
                                  <th class="py-1 pr-3">Verificado</th>
                                  <th class="py-1 pr-3">Enlace banco</th>
                                </tr>
                              </thead>
                              <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                @for (pago of cuota.pagos; track pago.pagoCuotaId) {
                                  <tr>
                                    <td class="py-2 pr-3 text-slate-700 dark:text-slate-300">{{ pago.fechaPago }}</td>
                                    <td class="py-2 pr-3 text-right font-semibold text-slate-900 dark:text-white">S/ {{ formatMoney(pago.montoPago) }}</td>
                                    <td class="py-2 pr-3 text-slate-700 dark:text-slate-300">{{ pago.banco }}</td>
                                    <td class="py-2 pr-3 font-mono text-slate-700 dark:text-slate-300">{{ pago.numeroOperacion || '-' }}</td>
                                    <td class="py-2 pr-3" [class]="pago.verificadoBanco ? 'text-emerald-600 dark:text-emerald-300' : 'text-slate-500 dark:text-slate-400'">{{ pago.verificadoBanco ? 'Sí' : 'No' }}</td>
                                    <td class="py-2 pr-3 text-slate-700 dark:text-slate-300">{{ pago.pagoBancarioId || '-' }}</td>
                                  </tr>
                                } @empty {
                                  <tr>
                                    <td colspan="6" class="py-3 text-center text-slate-500 dark:text-slate-400">Sin pagos registrados para esta cuota.</td>
                                  </tr>
                                }
                              </tbody>
                            </table>
