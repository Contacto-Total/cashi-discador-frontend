...
// src/app/core/services/sip.service.ts
var import_jssip = __toESM(require_JsSIP());
var CallState;
(function(CallState2) {
...
var _SipService = class _SipService {
  constructor() {
    this.ua = null;
    ...
    this.blockIncomingCalls = false;
    this.onCallStatus = new EventEmitter();
    this.onError = new EventEmitter();
    this.onRegistered = new EventEmitter();
    this.onIncomingCall = new EventEmitter();
    this.onOutgoingCall = new EventEmitter();
  ...
  enableAutoAnswer() {
    ...
    console.log("\u{1F916} Auto-answer enabled for admin supervision");
  }
  /**
   ...
   */
  setCurrentOutgoingNumber(phoneNumber) {
    this.currentOutgoingNumber = phoneNumber;
    console.log("\u{1F4E4} [SipService] N\xFAmero de llamada saliente establecido:", phoneNumber);
    this.onOutgoingCall.emit({ to: phoneNumber });
  }
  /**
   * Initialize and register to FreeSWITCH
   */
  register(extension, password, wsServer, domain) {
    return __async(this, null, function* () {
      return new Promise((resolve, reject) => __async(this, null, function* () {
        try {
          this.currentExtension = extension;
          console.log("\u{1F50C} Registering to FreeSWITCH:", { extension, wsServer, domain });
          ...
          const socket = new import_jssip.default.WebSocketInterface(wsUrl);
          socket.onconnect = () => {
            console.log("\u{1F517} WebSocket onconnect event fired");
          };
          socket.ondisconnect = (error, code, reason) => {
            console.error("\u{1F50C} WebSocket ondisconnect event fired:", { error, code, reason });
            if (!isResolved) {
              isResolved = true;
          ...
          this.ua = new import_jssip.default.UA(configuration);
          ...
          this.ua.on("registered", () => {
            ...
            if (this.currentExtension) {
              console.log(`\u{1F4E1} Starting latency monitoring for extension ${this.currentExtension}`);
          ...
          this.ua.on("newRTCSession", (data) => {
            ...
            if (session.direction === "incoming") {
              console.log("\u{1F4F2} Incoming call from:", session.remote_identity.uri.user);
              if (this.blockIncomingCalls) {
                console.warn("\u{1F6AB} REJECTING incoming call - agent is tipifying (ACW period)");
                session.terminate({
                  status_code: 486,
              ...
              const hasAutoAnswer = request && (request.hasHeader("Alert-Info") || request.hasHeader("Call-Info") || request.hasHeader("Answer-After"));
              if (this.autoAnswerEnabled || hasAutoAnswer) {
                console.log("\u{1F916} Auto-answering call (auto-dialer mode)");
          ...
          try {
            ...
            console.log("\u{1F3A4} Microphone access granted");
          } catch (error) {
  ...
  call(destination) {
    return __async(this, null, function* () {
      ...
      try {
        console.log("\u{1F4DE} Making call to:", destination);
        this.currentCallState = CallState.CONNECTING;
        this.onCallStatus.emit(this.currentCallState);
        const options = {
          mediaConstraints: {
          ...
          pcConfig: {
            ...
            // Disable pre-gathering for faster startup
          },
        ...
        console.log("\u{1F4E4} Emitiendo onOutgoingCall con destino:", destination);
  ...
  answer() {
    return __async(this, null, function* () {
      ...
      try {
        console.log("\u{1F4F2} Answering call");
        const options = {
          mediaConstraints: {
            audio: true,
            video: false
  ...
  hangup() {
    ...
    try {
      console.log("\u{1F4F4} Hanging up call");
  ...
  mute() {
    if (this.currentSession) {
      ...
      console.log("\u{1F507} Muted");
    }
  }
  unmute() {
    if (this.currentSession) {
      this.currentSession.unmute({ audio: true });
      console.log("\u{1F50A} Unmuted");
    }
  }
  isMuted() {
   ...
   */
  setupSDPOptimization(session) {
    session.on("sdp", (data) => {
      if (data.originator === "local") {
        let sdp = data.sdp;
        const originalLength = sdp.length;
        console.log("\u{1F527} Filtering LOCAL SDP - removing problematic candidates");
   ...
   */
  setupICEGatheringMonitor(session, callType) {
    console.log(`\u23F1\uFE0F Starting ICE gathering monitor for ${callType} call`);
    let gatheringStartTime;
    let gatheringTimeout = null;
    let candidateCount = 0;
    let hasSrflxCandidate = false;
    let hasHostCandidate = false;
    const handlePeerConnection = (pc) => {
      gatheringStartTime = Date.now();
      console.log("\u{1F50D} ICE Gathering State:", pc.iceGatheringState);
      const originalOnIceCandidate = pc.onicecandidate;
      console.log("\u{1F527} Installing aggressive ICE candidate filter");
      pc.onicecandidate = (event) => {
        if (event.candidate) {
          const candidate = event.candidate;
          const sdp = candidate.candidate;
          if (sdp.includes(":") && !sdp.includes("::ffff:")) {
            console.log("\u{1F6AB} Blocked IPv6 candidate from being added");
            return;
          }
          if (candidate.protocol === "tcp" || sdp.toLowerCase().includes(" tcp ")) {
            console.log("\u{1F6AB} Blocked TCP candidate from being added");
            return;
          }
          if (sdp.includes("172.28.") || sdp.includes("172.16.") || sdp.includes("169.254.") || sdp.includes("172.17.")) {
            console.log("\u{1F6AB} Blocked virtual/APIPA candidate from being added");
            return;
          }
      ...
      console.log("\u{1F527} [AUDIO] Setting up remote audio track handlers");
      pc.addEventListener("track", (event) => {
        console.log("\u{1F3B5} Received remote audio track:", {
        ...
        if (this.remoteAudio && event.streams[0]) {
          ...
          this.remoteAudio.play().then(() => console.log("\u2705 Playing remote audio")).catch((err) => console.error("\u274C Error playing audio:", err));
        }
      });
      ...
      const trackCheckInterval = setInterval(() => {
        ...
        if (hasTrack && this.remoteAudio && !this.remoteAudio.srcObject) {
          console.log(`\u{1F3B5} Found remote track on check #${trackCheckCount}`);
          const remoteStream = new MediaStream(receivers.map((r) => r.track).filter((t) => t !== null));
          if (remoteStream.getTracks().length > 0) {
            this.remoteAudio.srcObject = remoteStream;
            this.remoteAudio.play().then(() => console.log("\u2705 Playing remote audio from monitoring")).catch((err) => console.error("\u274C Error playing audio:", err));
      ...
      pc.addEventListener("icegatheringstatechange", () => {
        ...
        console.log(`\u{1F504} ICE Gathering State changed to: ${pc.iceGatheringState} (${elapsed}ms elapsed)`);
      ...
      pc.addEventListener("iceconnectionstatechange", () => {
        console.log(`\u{1F50C} ICE Connection State: ${pc.iceConnectionState}`);
      ...
      pc.addEventListener("connectionstatechange", () => {
        console.log(`\u{1F517} PeerConnection State: ${pc.connectionState}`);
      ...
      pc.addEventListener("icecandidate", (event) => {
        if (event.candidate) {
          ...
          if (candidate.type === "srflx") {
            ...
            setTimeout(() => {
              if (pc.iceGatheringState !== "complete") {
                console.log("\u{1F527} FORCE COMPLETING ICE gathering - dispatching null candidate manually");
          ...
          console.log(`\u{1F9CA} ICE Candidate #${candidateCount} (${elapsed}ms):`, {
            type: candidate.type,
          ...
          const elapsed = Date.now() - gatheringStartTime;
          console.log(`\u{1F3C1} ICE gathering signaled completion in ${elapsed}ms`);
          if (gatheringTimeout) {
            clearTimeout(gatheringTimeout);
          }
        }
      ...
      gatheringTimeout = setTimeout(() => {
        const elapsed = Date.now() - gatheringStartTime;
        if (pc.iceGatheringState !== "complete") {
          console.error(`\u{1F6A8} ICE gathering STILL not complete after ${elapsed}ms (state: ${pc.iceGatheringState})`);
          console.error(`\u{1F6A8} This is causing the call to fail - gathering should complete in < 2 seconds`);
          console.warn(`\u{1F4CA} Status: ${candidateCount} candidates, srflx=${hasSrflxCandidate}, host=${hasHostCandidate}`);
    ...
    if (session.connection) {
      console.log("\u{1F527} [FIX] PeerConnection already exists (outgoing call), setting up directly");
      handlePeerConnection(session.connection);
    } else {
      console.log("\u{1F527} [FIX] PeerConnection not yet created (incoming call), waiting for event");
      session.once("peerconnection", (data) => {
  ...
  setupSessionHandlers(session) {
    session.on("progress", () => {
      console.log("\u{1F4DE} Call progress (ringing)");
      this.currentCallState = CallState.RINGING;
      this.onCallStatus.emit(this.currentCallState);
    });
    session.on("accepted", () => {
  ...
  unregister() {
    return __async(this, null, function* () {
      ...
      try {
        console.log("\u{1F44B} Unregistering from FreeSWITCH");
        this.stopRingtone();
        if (this.currentSession) {
          this.currentSession.terminate();
        }
...
_SipService.\u0275fac = function SipService_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _SipService)();
};
_SipService.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _SipService, factory: _SipService.\u0275fac, providedIn: "root" });
var SipService = _SipService;
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(SipService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
//# sourceMappingURL=chunk-FHH72WSF.js.map
