...
var _WebsocketService = class _WebsocketService {
  ...
  subscribeToTopic(topic, subject) {
    if (!this.stompClient?.connected) {
      console.warn(`[WebSocket] Cannot subscribe to ${topic}: STOMP client not connected`);
      return;
    }
    try {
      console.log(`[WebSocket] \u{1F50C} Subscribing to topic: ${topic}`);
      this.stompClient.subscribe(topic, (message) => {
        try {
          console.log(`[WebSocket] \u{1F4E8} Raw message on ${topic}:`, message.body);
          const payload = JSON.parse(message.body);
          console.log(`[WebSocket] \u{1F4E6} Parsed payload on ${topic}:`, payload);
   ...
   */
  startLatencyMonitoring(extension) {
    this.currentExtension = extension;
    this.subscribe("/user/queue/latency").subscribe((pongMessage) => {
      if (pongMessage.type === "PONG" && pongMessage.clientTimestamp) {
        const now = Date.now();
        const rtt = now - pongMessage.clientTimestamp;
        console.log(`\u{1F4E1} Latency for ${extension}: ${rtt} ms`);
...
_WebsocketService.\u0275fac = function WebsocketService_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _WebsocketService)(\u0275\u0275inject(AuthService));
};
...
//# sourceMappingURL=chunk-NSDE3IOW.js.map
